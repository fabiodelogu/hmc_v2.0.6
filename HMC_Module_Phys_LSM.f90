...
contains
	...
	! Net Radiation [W/m^2] --> sigma [W/m^2 K^4], EpsA [%], EpsS [%], albedo [-], Ta [K], LST [K], K [W/m^2]
        ...
        dVarLST = sum(a2dVarLSTUpd)/max(1,count(a2dVarDem.gt.0.0))
        dVarH = sum(a2dVarH)/max(1,count(a2dVarDem.gt.0.0))
        dVarLE = sum(a2dVarLE)/max(1,count(a2dVarDem.gt.0.0))
        dVarRn = sum(a2dVarRn)/max(1,count(a2dVarDem.gt.0.0))

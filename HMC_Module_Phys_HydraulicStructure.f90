...
contains
    ...
    subroutine HMC_Phys_Dam_Spilling(iID, iNDam, dDt, &
        ...
        ! Variable(s)
        integer(kind = 4)           :: iID
        integer(kind = 4)           :: iI, iJ, iD
        integer(kind = 4)           :: iNDam
        real(kind = 4)              :: dDt
        
        real(kind = 4), dimension (iNDam)           :: a1dVarVDam, a1dVarQoutDam, a1dVarCoeffDam
        !------------------------------------------------------------------------------------------
        ...
        if (iNDam .gt. 0) then
            ...
            do iD = 1, iNDam
                ...
                !------------------------------------------------------------------------------------------
                ! Check limit(s)
                if ( (a1dVarCoeffDam(iD) .gt. 0.0) .and. &
                     (a1dVarVDam(iD) .gt. oHMC_Namelist(iID)%dTV*oHMC_Vars(iID)%a1dVMaxDam(iD)) ) then  
		    ...
		    !------------------------------------------------------------------------------------------
                    ...
                    !------------------------------------------------------------------------------------------
                    ! Discharge in [m^3/s]
                    a1dVarQoutDam(iD) = (a1dVarVDam(iD) - &
                                        oHMC_Namelist(iID)%dTV*oHMC_Vars(iID)%a1dVMaxDam(iD))*a1dVarCoeffDam(iD) 
                    
                    if ( a1dVarQoutDam(iD).gt.oHMC_Vars(iID)%a1dQcSLDam(iD) ) a1dVarQoutDam(iD) = oHMC_Vars(iID)%a1dQcSLDam(iD)

                    ! Update dam volume [m^3]
        ...
        if (iNDam .gt. 0) then
            ...
            do iD = 1, iNDam
                ...
                if ( a1dVarVDam(iD) .gt. oHMC_Namelist(iID)%dTV*oHMC_Vars(iID)%a1dVMaxDam(iD) ) then
                    ...
                    !iRank = size(oHMC_Vars(iID)%a2dVDam(iD,:), dim=1)

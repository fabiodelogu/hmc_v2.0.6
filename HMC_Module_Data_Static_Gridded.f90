...
module HMC_Module_Data_Static_Gridded
    ...
    use HMC_Module_Tools_Generic,   only:   nullborder2DVar
...
contains
    ...
    subroutine HMC_Data_Static_Gridded_Land(iID, iRows, iCols)
        ...
        real(kind = 4),     dimension (iRows, iCols)    :: a2dVarCt, a2dVarCf, a2dVarUc, a2dVarUh 
        real(kind = 4),     dimension (iRows, iCols)    :: a2dVarCoeffResol
        ...
        a2dVarCt = 0.0; a2dVarCf = 0.0; a2dVarUc = 0.0; a2dVarUh = 0.0;  a2dVarCoeffResol = 0.0
        a1dVarFCN = 0.0
        !------------------------------------------------------------------------------------------
        ...
        if (iTypeData == 2) then
            ...
            if ( .not. bFileExist ) then
                ...
                call HMC_Tools_IO_Get2d_NC(sVarName, iFileID, a2dVar, sVarUnits, iCols, iRows, .false., iErr)
                if (iErr /= 0) then
                    call mprintf(.true., iWARN, ' CoeffRes data not found. Initializing CoeffRes with default values')
                    where(a2dVarDEM.gt.0.0)
                        a2dVarCoeffResol = 1
                    elsewhere
        ...
        !------------------------------------------------------------------------------------------
        ! Check parameters maps
        where (a2dVarCt.lt.0.0 .and. a2iVarMask.gt.0)
            a2dVarCt = dCt
        endwhere
        where (a2dVarCf.lt.0.0 .and. a2iVarMask.gt.0)
            a2dVarCf = dCf
        endwhere
        where (a2dVarUh.lt.0.0 .and. a2iVarMask.gt.0)
            a2dVarUh = dUh
        endwhere
        where (a2dVarUc.lt.0.0 .and. a2iVarMask.gt.0)
            a2dVarUc = dUc
        endwhere
	...
	where(a2dVarDem.gt.0.0 .and. a2dVarBeta.le.0.0)
        ...
        dDxM = sqrt(sum(a2dVarAreaCell, mask=a2dVarAreaCell.gt.0.0) / count(a2dVarAreaCell.gt.0.0))
        dDyM = sqrt(sum(a2dVarAreaCell, mask=a2dVarAreaCell.gt.0.0) / count(a2dVarAreaCell.gt.0.0))
        ...
        if ( all(a2dVarCoeffResol.eq.1.0 )) then
            ...
            where ( a2dVarDem.gt.0.0 .and. a2iVarChoice.ge.0 ) 
                a2dVarCoeffResol = exp(-sqrt(a2dVarAreaCell)*0.0007)
            elsewhere
                a2dVarCoeffResol = 0.0
            endwhere
